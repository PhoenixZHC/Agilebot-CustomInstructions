...
def __cm_conn_log(msg: str):
    """连接诊断：宿主 logger + stdout + 日志文件（便于示教器导出/拷贝）。"""
    text = f"[CM连接] {msg}"
    try:
        logger.info(text)
    except Exception:
...
def __cm_log_instruction(name: str):
    """须用 wraps 保留原函数签名，否则扩展服务 HTTP 反射成 (*args,**kwargs) 会 400 缺少 args/kwargs。"""
...
def __cm_bootstrap_trace():
    """import 时执行：证明已加载 CM_oldsdk (SDK1.7)。"""
    ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    line = (
        f"{ts} [CM引导] Python 已加载 CM_oldsdk.py (SDK1.7)。"
        f"后续连接步骤见 [CM连接]；本行不依赖是否执行自定义指令。\n"
    )
    ok = []
    for p in __cm_candidate_log_paths():
        try:
    ...
    try:
        ...
        if ok:
            ...
            sys.stderr.write(
                "[CM引导] 所有候选日志路径均不可写，请设置环境变量 AGILEBOT_CM_LOG_FILE 指向 U 盘等可写路径。\n"
            )
        sys.stderr.flush()
    except Exception:
        pass
...
def __get_robot_ip():
    ...
    优先级：
    1. 环境变量 AGILEBOT_ROBOT_IP 或 ROBOT_IP（非空则直接使用，便于改IP后手动指定）
    2. Extension().get_robot_ip()（来自宿主/示教器侧配置）
    ...
    for env_key in ("AGILEBOT_ROBOT_IP", "ROBOT_IP"):
        ...
        if override:
            ...
            if override:
                __cm_conn_log(f"控制柜 IP 来自环境变量 {env_key}: {override}")
                return override
    try:
        extension = Extension()
        ...
        if robot_ip:
            __cm_conn_log(f"Extension.get_robot_ip() 返回控制柜 IP: {robot_ip}")
        else:
            __cm_conn_log("Extension.get_robot_ip() 返回空字符串或 None")
        return robot_ip if robot_ip else None
    except Exception as ex:
        logger.error(f"获取机器人IP失败: {ex}")
        __cm_conn_log(f"Extension.get_robot_ip() 异常: {ex}")
        return None


def __get_optional_teach_panel_ip():
    """
    connect(arm_controller_ip, teach_panel_ip) 第二参数（与官方 Python SDK 4.1-arm 一致）。
    若当前 1.7 SDK 的 connect 仅支持单参数，将在 __arm_connect 中自动回退。
    """
    for env_key in ("AGILEBOT_TEACH_PANEL_IP", "TEACH_PANEL_IP"):
        ...
        if v:
            v = str(v).strip()
            if v:
                __cm_conn_log(f"示教器 IP 来自环境变量 {env_key}: {v}")
                return v
    return None
...
def __create_arm_instance():
    """
    Arm(local_proxy=...) 与官方文档一致；当前 SDK 若不支持该参数则回退为 Arm()。
    环境变量 AGILEBOT_ARM_LOCAL_PROXY: true/1/yes/on 为 True；false/0/no/off 为 False。
    """
    raw = os.environ.get("AGILEBOT_ARM_LOCAL_PROXY")
    if raw is None:
        __cm_conn_log("创建 Arm() 默认 local_proxy（未设置 AGILEBOT_ARM_LOCAL_PROXY）")
        return Arm()
    s = str(raw).strip().lower()
    try:
        if s in ("1", "true", "yes", "on"):
            __cm_conn_log("创建 Arm(local_proxy=True)（环境变量 AGILEBOT_ARM_LOCAL_PROXY）")
            return Arm(local_proxy=True)
        if s in ("0", "false", "no", "off"):
            __cm_conn_log("创建 Arm(local_proxy=False)（环境变量 AGILEBOT_ARM_LOCAL_PROXY）")
            return Arm(local_proxy=False)
    except TypeError:
        logger.warning("当前 SDK 的 Arm 不支持 local_proxy 参数，已使用 Arm()")
        __cm_conn_log("Arm(local_proxy=...) 不支持，已回退 Arm()")
    __cm_conn_log("创建 Arm() 默认 local_proxy")
    return Arm()
...
def __arm_connect(arm, controller_ip: str, teach_panel_ip):
    if teach_panel_ip:
        ...
        try:
            ...
            logger.warning("当前 SDK 的 connect 不支持第二参数，已回退为 connect(控制柜IP)")
            __cm_conn_log("connect 第二参数不支持，回退 connect(控制柜IP)")
...
def __arm_is_connected(arm):
    """优先 is_connected()（新版），否则 is_connect()（1.7 旧名）。"""
    ...
    for name in ("disconnect", "Disconnect", "close", "Close"):
        ...
        if callable(fn):
            try:
                ...
                logger.warning(f"[CM连接] {name}() 异常: {ex}")
...
def __get_arm_connection():
    """
    获取Arm连接（长连接机制）
    如果未连接则连接，已连接则复用；若控制柜/示教器连接参数变化则先断开再连。
    ...
    try:
        ...
        teach_opt = __get_optional_teach_panel_ip()
            ...
            if _global_connect_fingerprint is not None and fp != _global_connect_fingerprint:
                ...
                try:
                    if __arm_is_connected(_global_arm):
                        ...
                        __cm_conn_log("复用已有连接，在线检查为 True")
                        return _global_arm, None
                except Exception as ex:
                    __cm_conn_log(f"在线状态检查异常，将重连: {ex}")
        ...
        if ret != StatusCodeEnum.OK:
            __disconnect_global_arm()
            __cm_conn_log(f"connect 失败，错误代码: {ret}")
...
def __create_r_register(arm, r_id: int, initial_value: float = 0.0):
    ...
    try:
        ...
        if ret == StatusCodeEnum.OK:
            ...
            logger.error(f"创建R寄存器[{r_id}]失败，错误代码：{ret}")
            return ret, False
            ...
            if hasattr(pr_register, 'poseRegisterData') and \
               ...
               hasattr(pr_register.poseRegisterData.cartData, 'position'):
                ...
                if ret == StatusCodeEnum.OK:
                    ...
                    logger.error(f"写入PR寄存器[{pr_id}]失败，错误代码：{ret}")
                    return None, ret
            else:
...
def Strp(SR_ID: int, R_ID_Status: int, PR_ID: int, R_ID_Error: int) -> dict:
    ...
    try:
        ...
        if ret != StatusCodeEnum.OK:
            logger.error(f"读取SR寄存器[{SR_ID}]失败，错误代码：{ret}")
            return {"success": False, "error": f"读取SR寄存器[{SR_ID}]失败，错误代码：{ret}"}
        ...
        if ret != StatusCodeEnum.OK:
            logger.warning(f"写入R_ID_Status寄存器[{R_ID_Status}]失败，错误代码：{ret}")
        ...
        while pr_count < num_pr_registers:
            ...
            if ret != StatusCodeEnum.OK:
                logger.error(f"PR寄存器[{current_pr_id}]不存在（错误代码：{ret}）")
                __create_r_register(arm, R_ID_Error, 1)
                arm.register.write_R(R_ID_Error, 1)
                return {
                    "success": False,
            ...
            if ret != StatusCodeEnum.OK:
                logger.error(f"写入PR寄存器[{current_pr_id}]失败，错误代码：{ret}")
                __create_r_register(arm, R_ID_Error, 1)
                arm.register.write_R(R_ID_Error, 1)
                return {
                    "success": False,
                    "error": f"写入PR寄存器[{current_pr_id}]失败，错误代码：{ret}。请检查：1) PR寄存器是否存在 2) PR寄存器是否被锁定 3) 数据格式是否正确"
                }
...
def TFShift(InputTF_ID: int = 1, ResultTF_ID: int = 3, CamPose_ID: int = 60, RefVis_ID: int = 61, ActVis_ID: int = 62) -> dict:
    ...
    try:
        ...
        tool, ret = arm.coordinate_system.get(CoordinateSystemType.ToolFrame, InputTF_ID)
        if ret != StatusCodeEnum.OK:
            return {"success": False, "error": f"读取基准工具坐标系[{InputTF_ID}]失败，错误代码：{ret}"}
        tool_data = [
            tool.position.x,
            tool.position.y,
            tool.position.z,
        ...
        if ret != StatusCodeEnum.OK:
            return {"success": False, "error": f"读取拍照点PR寄存器[{CamPose_ID}]失败，错误代码：{ret}"}
        if not hasattr(pr_register, 'poseRegisterData') or \
           not hasattr(pr_register.poseRegisterData, 'cartData') or \
           not hasattr(pr_register.poseRegisterData.cartData, 'position'):
        ...
        if ret != StatusCodeEnum.OK:
            return {"success": False, "error": f"读取基准视觉模板PR寄存器[{RefVis_ID}]失败，错误代码：{ret}"}
        if not hasattr(pr_register, 'poseRegisterData') or \
           not hasattr(pr_register.poseRegisterData, 'cartData') or \
           not hasattr(pr_register.poseRegisterData.cartData, 'position'):
        ...
        if ret != StatusCodeEnum.OK:
            return {"success": False, "error": f"读取实际视觉坐标PR寄存器[{ActVis_ID}]失败，错误代码：{ret}"}
        if not hasattr(pr_register, 'poseRegisterData') or \
           not hasattr(pr_register.poseRegisterData, 'cartData') or \
           not hasattr(pr_register.poseRegisterData.cartData, 'position'):
        ...
        coordinate, ret = arm.coordinate_system.get(CoordinateSystemType.ToolFrame, ResultTF_ID)
        if ret != StatusCodeEnum.OK:
            return {"success": False, "error": f"获取工具坐标系[{ResultTF_ID}]失败，错误代码：{ret}"}

        coordinate.position.x = ut2_pose_list[0]
        coordinate.position.y = ut2_pose_list[1]
        coordinate.position.z = ut2_pose_list[2]
        coordinate.orientation.r = ut2_pose_list[3]
        coordinate.orientation.p = ut2_pose_list[4]
        coordinate.orientation.y = ut2_pose_list[5]

        ret = arm.coordinate_system.update(CoordinateSystemType.ToolFrame, coordinate)
        if ret != StatusCodeEnum.OK:
            return {"success": False, "error": f"更新工具坐标系[{ResultTF_ID}]失败，错误代码：{ret}"}

        return {
            "success": True,
            "message": f"工具坐标系补正完成，结果已写入TF[{ResultTF_ID}]：X={ut2_pose_list[0]:.6f}, Y={ut2_pose_list[1]:.6f}, Z={ut2_pose_list[2]:.6f}, W={ut2_pose_list[3]:.6f}, P={ut2_pose_list[4]:.6f}, R={ut2_pose_list[5]:.6f}"
        }
...
def DecToHex(R_ID: int, SR_ID: int) -> dict:
    ...
    try:
        ...
        r_value, ret = arm.register.read_R(R_ID)
        if ret != StatusCodeEnum.OK:
            logger.error(f"读取R寄存器[{R_ID}]失败，错误代码：{ret}")
            return {"success": False, "error": f"读取R寄存器[{R_ID}]失败，错误代码：{ret}"}
        ...
        if ret != StatusCodeEnum.OK:
            logger.error(f"写入SR寄存器[{SR_ID}]失败，错误代码：{ret}")
            return {"success": False, "error": f"写入SR寄存器[{SR_ID}]失败，错误代码：{ret}"}
